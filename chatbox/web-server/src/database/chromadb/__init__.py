...
class ChromaClient:
    ...
    def __new__(cls):
        ...
        if flask_env == "development" or not flask_env:
            ...
            print("ccccccccccc",os.environ["CHROMA_URL_DOCKER"])
            if cls._instance is None:
                cls._instance = chromadb.HttpClient(
                    host=os.environ["CHROMA_URL_DOCKER"], port=os.environ["CHROMA_PORT"])
            return cls._instance
            ...
            if cls._instance is None:
                cls._instance = chromadb.PersistentClient(
                    path=os.environ.get('CHROMA_DB_PATH'))
            return cls._instance
        elif not flask_env and os.environ["CHROMA_URL_DOCKER"] == "your_chromadb_url":
            print("keke")
            if cls._instance is None:
                cls._instance = chromadb.PersistentClient(
                    path=os.environ.get('CHROMA_DB_PATH'))
